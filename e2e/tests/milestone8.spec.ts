...
// Fake-Preise: BTC 50.000 € · SOL 100 € · fookoin 2 €
...
test('CSV-Transaktions-Import: Typen/Daten validiert, Netto-Bestände berechnet', async ({ page }) => {
  ...
  await page.getByTestId('csv-file').setInputFiles({
    ...
    buffer: Buffer.from(
      'Datum;Typ;Coin;Menge\n' +
        ...
        '2024-04-01;hodl;SOL;5\n', // Zeile 5: unbekannter Typ
      'utf8',
    ),
  })
  await page.getByTestId('csv-upload').click()

  // Alle vier Spalten automatisch zugeordnet
  ...
  // Netto: BTC 1 − 0,4 = 0,6 (30.000 €) · SOL +10 (1.000 €)
...
test('Allocation-Donut zeigt Anteile der Top-Positionen', async ({ page }) => {
  ...
  // 1 BTC (50.000 €) + 10 SOL (1.000 €) manuell erfassen
  await page.getByRole('tab', { name: 'Bestände' }).click()
...
test('manuelles Preis-Mapping über die CoinGecko-Suche', async ({ page }) => {
  ...
  // Asset-Mappings sind global und die E2E-DB persistiert über Läufe —
  // deshalb ein lauf-eindeutiges Symbol (Echo-Fake liefert dazu eine eindeutige ID)
  ...
  // Mapping: Suche ist mit dem Symbol vorbefüllt → Echo-Treffer wählen
  ...
  // Jetzt bepreist mit Fake-Default 1 €: 5 × 1 € = 5 €
