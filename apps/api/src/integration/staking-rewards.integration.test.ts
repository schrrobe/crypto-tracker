...
// FAKE_PROVIDERS: Solana wallet returns 12 SOL + 2 deterministic rewards
// (fake-sol-reward:1/2) — sync is idempotent via externalRef
...
describe('On-Chain-Staking-Rewards (Integration)', () => {
  it('Sync erzeugt Reward-Transaktionen, zweiter Sync dupliziert nicht', async () => {
    ...
    // Idempotency: a repeated sync creates nothing new
    ...
    // Holdings stay snapshot-based (12 SOL from the fake provider, no reward addition)
    const holdings = await prisma.holding.findMany({ where: { sourceId: source.id } })
    expect(holdings).toHaveLength(1)
    expect(holdings[0]?.quantity.toString()).toBe('12')

    // Rewards appear in the transaction list, not editable
  ...
  it('DE-Report: Reward-Zufluss als Staking-Einkommen + WALLET_REWARDS_ONLY-Hinweis', async () => {
    ...
    // Fake rewards: 0.05 + 0.07 SOL in March 2024, price via fake backfill
